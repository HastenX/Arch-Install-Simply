...
function install_sh() {
    ...
    if [[ $test != "y" ]]; then
        ...
        updateGrub
        if [[ ${UserBuild[encrypt]} != "-1" && ${UserBuild[personalKey]} == "-1" ]]; then
            updateCrypttab
        fi
...
function encrypt() {
    if [[ ${UserBuild[encryption]} != "-1" && ${UserBuild[personalKey]} != "-1" ]]; then 
        cryptsetup luksFormat "/dev/${UserBuild[lvmPartition]}"
        cryptsetup open --type luks "/dev/${UserBuild[lvmPartition]}" lvm
        return
    fi
    if [[ ${UserBuild[encryption]} != "-1" && ${UserBuild[personalKey]} == "-1" ]]; then 
        (echo "ex"; echo "ex";) | cryptsetup luksFormat "/dev/${UserBuild[lvmPartition]}"
        (echo "ex";) | cryptsetup open --type luks /dev/${UserBuild[lvmPartition]} lvm
        return
    fi
}
